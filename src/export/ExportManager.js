...
export class ExportManager {
  ...
  async getJpeg() {
    ...
    const imageData = await new Promise(resolve => {
      this.scratchCanvas.toBlob((blob) => {
        resolve(blob);
      }, "image/jpeg", DEFAULT_VALUES.JPEG_QUALITY);
    });
    return imageData;
  }
  ...
  async takePicture() {
    ...
    if (this.eventTarget) {
      this.eventTarget.dispatchEvent(new CustomEvent(EVENTS.NEW_PICTURE, {
        detail: img
      }));
    }
  }
   ...
   * Each layer represents one ink color, with black pixels indicating where ink should be applied
   *
   ...
   * @param {number} eyeSize - Canvas dimensions
   */
  async exportRisographLayers(color1, color2, color3, color4, color5, eyeSize) {
    if (!this.finalCanvas) {
      throw new Error('Export manager not initialized');
    }
    ...
    const imageData = ctx.getImageData(0, 0, eyeSize, eyeSize);
